...
def create_folder(working_directory, all_files, map_type=None, step="Other", folder_directory=None):
    assert map_type == "Bwa" or map_type == "Bowtie2", "Program has encountered a fatal error! Please give the correct mapping type: Bwa or Bowtie2"  # newly added piece of code.
    ...
    if step == "Mapping":
        ...
        elif os.path.isdir(mk_dir):
            while os.path.isdir(folder_directory + "/" + step + str(version))==True:
                version+=1
            mk_dir = folder_directory + "/" + step + str(version)
            os.mkdir(mk_dir)
            for file in all_files:
                if file[-2:] != "gz":
                    print("mapping crate folder print " + file)
        ...
        if not os.path.isdir(mk_dir):
            ...
            while os.path.isdir(mk_dir + "/"+ str(qc_version))==True:
                qc_version+=1
            mk_dir = mk_dir + str(qc_version)
            os.mkdir(mk_dir)
            mk_dir += "/" + step
            os.mkdir(mk_dir)
            for file in all_files:
